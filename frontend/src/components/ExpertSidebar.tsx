...
import { 
  LayoutDashboard, BookOpen, Award, Users, Bell, 
  MessageSquare, LogOut, ShieldCheck, Sprout, ChevronRight, X, ChevronLeft
} from 'lucide-react';
...
export const ExpertSidebar: React.FC<ExpertSidebarProps> = ({ 
  isOpen, 
  onClose,
  isCollapsed,
  onToggleCollapse
  ...
  const navItems = [
    ...
    {
      label: 'Mes Formations & Cours',
      path: '/expert/formations',
      icon: BookOpen
    },
    {
      label: 'Créer & Gérer Quiz',
      path: '/expert/formations/quiz',
      icon: Award,
      badge: 'Nouveau'
    },
    {
      label: 'Suivi des Apprenants',
      path: '/expert/apprenants',
      icon: Users
    },
    {
    ...
    {
      label: 'Forum & Réponses',
      path: '/expert/forum',
      icon: MessageSquare
    }
  ];

  // FIX: Évite que "Mes Formations" soit activé quand on est sur "/expert/formations/quiz"
  const isItemActive = (itemPath: string) => {
    if (itemPath === '/expert') {
      return location.pathname === '/expert' || location.pathname === '/expert/';
    }
    if (itemPath === '/expert/formations') {
      return (
        (location.pathname === '/expert/formations' || location.pathname === '/expert/formations/') &&
        !location.pathname.includes('/quiz')
      );
    }
    if (itemPath === '/expert/formations/quiz') {
    ...
    <>
      ...
      >
        ...
        <div className={`h-16 flex items-center border-b border-emerald-800/80 bg-emerald-950 px-4 transition-all ${
          ...
          <Link to="/expert" className="flex items-center space-x-3 group">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-6 space-y-6 scrollbar-thin">
          <div className="space-y-1.5">
            ...
            {navItems.map((item) => {
              ...
              if (isCollapsed) {
                // Version Icône Seule (Mode Replié style Accusoft Image 4)
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                  ...
                  >
                    ...
                    {item.badge && (
                      <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-400 rounded-full ring-2 ring-emerald-950" />
                    )}
              ...
              // Version Dépliée Complète (Style Moderne Accusoft Image 3)
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  ...
                  <div className="flex items-center space-x-1 shrink-0">
                    {item.badge && (
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
                        active ? 'bg-amber-400 text-amber-950' : 'bg-amber-400/90 text-amber-950'
                      }`}>
                        {item.badge}
                      </span>
                    )}
        ...
        <div className="p-3 border-t border-emerald-850 bg-emerald-950">
          {isCollapsed ? (
            // Mode replié : juste l'avatar et bouton déconnexion
              ...
              <Link
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs shadow-xs transition-all ${
                  location.pathname.includes('/profile') || location.pathname.includes('/profil')
                    ...
                    : 'bg-gradient-to-tr from-emerald-500 to-teal-400 text-white hover:scale-105'
                }`}
              >
                {user?.prenom?.[0] || 'E'}{user?.nom?.[0] || 'X'}
              </Link>
              <button
            ...
            // Mode déplié : profil complet avec carte stylisée
            <div className={`rounded-2xl p-3 border flex items-center justify-between transition-all ${
              ...
              <Link
                ...
                title="Accéder à mon profil expert"
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-emerald-300 truncate">
                    {user?.profilExpert?.specialite || 'Expert Agronome'}
                  </div>
                </div>
              </Link>
